...
def inputPlayerLetter():
    ...
    # the first element - the player’s letter; the second - computer's letter
    if letter == "X":
        return ["X", "O"]
    else:
        return ["O", "X"]
...
def isWinner(bo: List[str], le: str) -> bool:
    ...
    def isWinnerByRow(winning_sequence):
    ...
    def isWinnerByColumn(winning_sequence):
    ...
    def isWinnerByDiagonal(winning_sequence):
        """Check both diagonals of the board, to see if the player has won."""
        # form the diagonals
        up_to_down = [bo[index] for index in range(1, NUM_BOXES, BOARD_LENGTH + 1)]
        down_to_up = [
            bo[index] for index in range(BOARD_LENGTH, NUM_BOXES, BOARD_LENGTH - 1)
        ]
    ...
    return (
        isWinnerByRow(winning_sequence)
        or isWinnerByColumn(winning_sequence)
        or isWinnerByDiagonal(winning_sequence)
    )
...
def getPlayerMove(board):
    ...
    while player_move not in moves or not isSpaceFree(board, int(player_move)):
...
def play_game():
    ...
    def play_one_round(turn):
        """Controls the game loop through one round of Tic Tac Toe."""
        while not turn == "stop":
            if turn == "player":
                drawBoard(theBoard)
                # player makes move
